 ...
 *     opened it, per CommonMark. A four-backtick block that displays a
 *     triple-backtick doctest example therefore stays one block, and the
 *     snippets after it are still reached.
...
export function extractDoctests(source: string): DoctestBlock[] {
  ...
  while (i < lines.length) {
    ...
    const bodyStart = i + 1;
    let j = bodyStart;
    while (j < lines.length && !closesFence(lines[j], fenceLength)) {
      j += 1;
    }
