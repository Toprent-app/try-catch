...
```bash
npm install -D tsx
npx tsx --tsconfig examples/tsconfig.json examples/comprehensive-examples.ts
npx tsx --tsconfig examples/tsconfig.json examples/custom-reporter.ts
```

`--tsconfig examples/tsconfig.json` is required: the `@power-rent/try-catch`
specifier resolves to the local `src/` tree through the `paths` mapping in that
file, and without it the runtime looks for an installed package instead.

`comprehensive-examples.ts` defines its sections but calls none of them on
