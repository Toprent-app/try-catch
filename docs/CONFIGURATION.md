...
| `types` | `["node"]` |
| `ignoreDeprecations` | `6.0` (only TypeScript 6.0+ accepts this value; 5.x rejects it with TS5103) |
| `include` | `src/**/*` |
| `exclude` | `node_modules`, `dist` |

This config is also used by the ESLint TypeScript parser (`parserOptions.project`).

### `tsconfig.build.json` (published declarations)

Extends `tsconfig.json` and is the config `npm run build:types` compiles. It emits declarations only and excludes the test suite:
