...
Use the root entry point when you have no Sentry SDK or want to supply your own reporter. The built-in `NoopReporter` disables error *reporting* only — it sends nothing anywhere. Error *handling* is unchanged: `.unwrap()` still throws, `.error()` still returns the `Error`, and `.value()` still returns the configured default.
