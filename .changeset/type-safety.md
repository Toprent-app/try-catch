...
- `.then` typed `never` AND not installed at runtime when wrapping sync functions — use `.value()` / `.unwrap()` synchronously. `await new Try(syncFn)` now returns the `Try` instance itself (previously it would silently unwrap)
- `.default()` now returns a fresh `Try` instance instead of mutating. Holding a reference before `.default()` no longer sees subsequent `.report()`/`.tag()` calls made on the chain after `.default()`
